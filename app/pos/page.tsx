...
// ━━━ Order detail item (for View Items) ━━━
...
function POSPageInner() {
  ...
  // ━━━ Load order detail (items for a specific order) ━━━
  async function loadOrderDetail(orderId: string) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        console.error("Order detail API error:", data?.error || res.status);
        setOrderDetailItems([]);
        return;
      }
  ...
  return (
    <div className="pos-surface flex min-h-[100dvh] flex-col bg-white text-gray-900">
      ...
      {/* ━━━ ORDER DETAIL MODAL (View Items - sementara tunggu thermal printer) ━━━ */}
      {orderDetailOpen && (() => {
        const detailOrder = orders.find(o => o.id === orderDetailOpen);
        return (
            ...
            <div className="flex max-h-[85vh] w-full max-w-md flex-col rounded-t-2xl bg-white sm:rounded-2xl">
              <div className="flex items-center justify-between border-b px-4 py-3">
                <div>
                  ...
                  <div className="text-xs text-gray-400">
                    {detailOrder?.customer_name || "Walk-in"} · {detailOrder ? new Date(detailOrder.created_at).toLocaleTimeString("ms-MY", { hour: "2-digit", minute: "2-digit" }) : ""}
                  </div>
                </div>
                <button onClick={() => setOrderDetailOpen(null)} className="text-xl text-gray-400 hover:text-gray-600">×</button>
              </div>
              <div className="flex-1 overflow-y-auto px-4 py-3">
                {orderDetailLoading ? (
                  ...
                  <div className="flex items-center justify-center py-10">
                    <div className="text-sm text-gray-400">Tiada item dijumpai</div>
                  </div>
                ) : (
                      ...
                      <div key={idx} className="flex items-start justify-between gap-3 border-b border-gray-100 pb-3 last:border-0">
                        <div className="min-w-0 flex-1">
                          ...
                          {item.addon_names.length > 0 && (
                            <div className="mt-0.5 text-xs text-[#7F1D1D]">+ {item.addon_names.join(", ")}</div>
                          )}
                          {item.sugar_level && (
                            <div className="mt-0.5 text-xs text-gray-400">Sugar: {item.sugar_level}</div>
                          )}
                          <div className="mt-0.5 text-xs text-gray-400">
              ...
              <div className="border-t bg-gray-50 px-4 py-3">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => { if (orderDetailOpen) window.open(`/api/orders/receipt/${orderDetailOpen}`, "_blank", "width=420,height=720"); }}
                  ...
                  <button
                    ...
                    className="flex-1 rounded-lg bg-[#7F1D1D] py-2.5 text-sm font-medium text-white active:bg-[#6F1A1A]"
                  >
                    Tutup
                  </button>
                </div>
