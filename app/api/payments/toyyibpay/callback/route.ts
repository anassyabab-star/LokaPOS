...
async function handleCallback(req: Request) {
  try {
    ...
    const isPaid = isToyyibPaidStatus(payload);
    const isFailed = isToyyibFailedStatus(payload);
    ...
    return NextResponse.json({
      ...
      bill_code: payload.billcode || payload.billCode || payload.bill_code || null,
      transaction_id: payload.transaction_id || payload.transactionId || payload.refno || null,
    });
  } catch (error) {
